...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('travels', function (Blueprint $table) {
            ...
            $table->foreignId('bus_id')->constrained('buses')->onDelete('restrict')->onUpdate('cascade');
            $table->foreignId('driver_id')->constrained('drivers')->onDelete('restrict')->onUpdate('cascade');
            $table->timestamp('departure_time');
            $table->timestamp('arrival_time');
            $table->enum('status', ['Scheduled', 'Departed', 'Completed', 'Canceled', 'Delayed'])->default('Scheduled'); 
            $table->timestamps();
        });
    }
